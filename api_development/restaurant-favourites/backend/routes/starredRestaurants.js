...
router.post("/", (req, res) => {
  ...
  const isAlreadyStarred = STARRED_RESTAURANTS.some(
    (starredRestaurant) => starredRestaurant.restaurantId === id
  );

  if (isAlreadyStarred) {
    return res.status(409).json({ message: "Restaurant is already starred." });
  }
