...
router.post("/", (req, res) => {
  ...
  if (!name) {
    return res.status(400).json({ message: "A restaurant name is required." });
  }
...
router.delete("/:id", (req, res) => {
  ...
  const restaurantIndex = ALL_RESTAURANTS.findIndex(
  ...
  // Keep the exported array reference current for the starred-restaurants route.
  ALL_RESTAURANTS.splice(restaurantIndex, 1);
